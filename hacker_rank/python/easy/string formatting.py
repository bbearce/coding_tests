...
def print_formatted(number):
    # your code goes here
    binary_length = len('{0:b}'.format(number))
    for i in range(1,number+1):
        print('{0:d}'.format(i).rjust(binary_length),
              '{0:o}'.format(i).rjust(binary_length),
              '{0:x}'.format(i).rjust(binary_length).upper(),
              '{0:b}'.format(i).rjust(binary_length))
